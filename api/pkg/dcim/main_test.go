...
const testDBPort = 45433

func TestMain(m *testing.M) {
	cacheDir := os.Getenv("DCIM_TEST_CACHE_DIR")
	if cacheDir == "" {
		...
		cacheDir = filepath.Join(userCache, "dcim-test-pg")
	}
...
func setupTemplateDatabaseWithMigrations(pool *pgxpool.Pool) error {
	...
	_, err := pool.Exec(context.Background(), "UPDATE pg_database SET datistemplate = false WHERE datname = 'dcim'")
	if err != nil {
		return fmt.Errorf("failed to unmark dcim as template: %v", err)
	}

	trekApply(projectRoot)

	_, err = pool.Exec(context.Background(), "UPDATE pg_database SET datistemplate = true WHERE datname = 'dcim'")
	if err != nil {
		return fmt.Errorf("failed to mark dcim as template: %v", err)
	}
...
func createRoles(pool *pgxpool.Pool) {
	...
	roles := []dbRole{
		{name: "dcim_api", bypassrls: false},
		{name: "dcim_operator", bypassrls: true},
		{name: "dcim_owner", bypassrls: true},
	}
