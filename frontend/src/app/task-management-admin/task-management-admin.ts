...
export class TaskManagementAdminComponent implements AfterViewInit {
  ngAfterViewInit(): void {
    ...
    const tasks: Task[] = [
      {
        id: 1, title: "Replace broken harddisk",
        description: "Failed disk in Bay 3 of backup-srv-07 at Rack 123. Replace with Seagate Exos X18 (ST16000NM000J, 16 TB). The RAID controller shows the drive as failed since yesterday evening.",
        status: "In Progress", priority: "Critical", category: "Hardware",
        location: "DC Amsterdam-West · Rack 123", assignee: 1, due: "2026-03-20", created: "2026-03-15",
        notes: [
          { author: 1, text: "Arrived at rack. Disk bay 3 LED is solid red. Starting replacement procedure.", time: "2 hours ago" },
          { author: null, text: "Spare disk is available in storage room B, shelf 3. Serial: ZLR1N5JY.", time: "5 hours ago" },
        ],
      },
      {
        id: 2, title: "Check cooling unit — Row 5",
        description: "Temperature sensors in Row 5 are reporting 2°C above normal baseline. Inspect the cooling unit for potential blockage or fan failure.",
        status: "Ready", priority: "High", category: "Cooling",
        location: "DC Amsterdam-West · Hall A, Row 5", assignee: null, due: "2026-03-21", created: "2026-03-17",
        notes: [
          { author: null, text: "Monitoring dashboard shows temps rising over the past 48h. Not yet critical but trending up.", time: "1 day ago" },
        ],
      },
      {
        id: 3, title: "Inspect PDU — Hall A",
        description: "Routine quarterly inspection of the PDU in Hall A. Check all breakers, verify load balancing, and ensure no burnt contacts.",
        status: "Ready", priority: "Medium", category: "Power",
        location: "DC Amsterdam-West · Hall A", assignee: 2, due: "2026-03-25", created: "2026-03-16",
        notes: [],
      },
      {
        id: 4, title: "Replace network switch — Rack 87",
        description: "The Cisco Nexus switch in Rack 87 has intermittent port failures on ports 24-28. Replace with the new Arista unit from stock.",
        status: "In Progress", priority: "High", category: "Network",
        location: "DC Amsterdam-West · Rack 87", assignee: 4, due: "2026-03-19", created: "2026-03-14",
        notes: [
          { author: 4, text: "Migration window confirmed with NOC for tonight 22:00–02:00. Pre-staging the replacement switch now.", time: "3 hours ago" },
          { author: null, text: "NOC has been notified. Maintenance window approved.", time: "1 day ago" },
        ],
      },
      {
        id: 5, title: "Firmware update — UPS units Hall B",
        description: "Apply firmware v4.2.1 to all three Eaton UPS units in Hall B. Requires sequential update — do not update all at once.",
        status: "Review", priority: "Medium", category: "Power",
        location: "DC Amsterdam-West · Hall B", assignee: 3, due: "2026-03-22", created: "2026-03-13",
        notes: [
          { author: 3, text: "UPS-1 and UPS-2 updated successfully. UPS-3 scheduled for tomorrow morning. All readings normal after update.", time: "6 hours ago" },
        ],
      },
      {
        id: 6, title: "Install additional cameras — Entrance B",
        description: "Mount two new security cameras at Entrance B as per the security audit recommendations. Cabling is already in place.",
        status: "Blocked", priority: "Low", category: "Security",
        location: "DC Amsterdam-West · Entrance B", assignee: 5, due: "2026-03-28", created: "2026-03-10",
        notes: [
          { author: 5, text: "Cameras arrived but mounting brackets are the wrong model. Waiting for replacement brackets from supplier.", time: "2 days ago" },
          { author: null, text: "Supplier confirmed new brackets ship Monday.", time: "1 day ago" },
        ],
      },
      {
        id: 7, title: "Decommission server DB-14",
        description: "Server DB-14 in Rack 45 has been migrated to new hardware. Wipe disks, remove from rack, and update asset inventory.",
        status: "Done", priority: "Low", category: "Hardware",
        location: "DC Amsterdam-West · Rack 45", assignee: 1, due: "2026-03-17", created: "2026-03-08",
        notes: [
          { author: 1, text: "Disks wiped with DBAN (3-pass). Server removed from rack and placed in decommission staging. Asset inventory updated.", time: "1 day ago" },
        ],
      },
      {
        id: 8, title: "Repair cable management — Rack 92",
        description: "Cables in Rack 92 are obstructing airflow. Re-route and zip-tie all patch cables. Replace any damaged cables.",
        status: "In Progress", priority: "Medium", category: "Hardware",
        location: "DC Amsterdam-West · Rack 92", assignee: 2, due: "2026-03-23", created: "2026-03-16",
        notes: [],
      },
    ];
    ...
    const statusStyles: Record<string, StatusStyle> = {
      "Ready":       { bg: "bg-slate-100",   text: "text-slate-600",   dot: "bg-slate-400",   kanbanAccent: "bg-slate-400",   kanbanBorder: "border-slate-200" },
      "In Progress": { bg: "bg-indigo-50",   text: "text-indigo-700",  dot: "bg-indigo-500",  kanbanAccent: "bg-indigo-500",  kanbanBorder: "border-indigo-200" },
      "Review":      { bg: "bg-amber-50",    text: "text-amber-700",   dot: "bg-amber-500",   kanbanAccent: "bg-amber-500",   kanbanBorder: "border-amber-200" },
      "Blocked":     { bg: "bg-red-50",      text: "text-red-700",     dot: "bg-red-500",     kanbanAccent: "bg-red-500",     kanbanBorder: "border-red-200" },
      "Done":        { bg: "bg-emerald-50",  text: "text-emerald-700", dot: "bg-emerald-500", kanbanAccent: "bg-emerald-500", kanbanBorder: "border-emerald-200" },
    };

    const priorityStyles: Record<string, PriorityStyle> = {
      "Critical": { bg: "bg-red-50",    text: "text-red-700",    dot: "bg-red-500",    ring: "ring-red-200/80" },
      "High":     { bg: "bg-orange-50", text: "text-orange-700", dot: "bg-orange-500", ring: "ring-orange-200/80" },
      "Medium":   { bg: "bg-yellow-50", text: "text-yellow-700", dot: "bg-yellow-400", ring: "ring-yellow-200/80" },
      "Low":      { bg: "bg-slate-100", text: "text-slate-500",  dot: "bg-slate-400",  ring: "ring-slate-200/80" },
    };
    ...
    function avatarHTML(tech: Technician | null, size = "h-7 w-7 text-xs"): string {
      if (!tech) return `<span class="inline-flex ${size} items-center justify-center rounded-full bg-slate-200 text-slate-500 font-medium shrink-0"><nldd-icon name="person" class="w-3 h-3"></nldd-icon></span>`;
    ...
    function openDetail(id: number): void {
      ...
      (document.getElementById("detailCreated") as HTMLElement).textContent     = formatDate(task.created);
      ...
      // Notes
      (document.getElementById("detailNoteCount") as HTMLElement).textContent = `(${task.notes.length})`;
      let notesHTML = "";
    ...
    (document.getElementById("editSaveBtn") as HTMLElement).addEventListener("click", () => {
      ...
      if (editingTaskId !== null) {
        ...
        tasks.push({ id: Date.now(), ...data, created: new Date().toISOString().split("T")[0], notes: [] });
    ...
    editModal.addEventListener("click", (e: Event) => { if (e.target === editModal) closeModal(editModal); });
