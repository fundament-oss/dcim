...
export class PatchMappingComponent {
  ...
  saveConnection(): void {
    ...
    if (!form) return;
    const srcDevice = (this.fSrcDevice()?.nativeElement as any)?.value as string;
    const srcPort   = (this.fSrcPort()?.nativeElement as any)?.value as string;
    const tgtDevice = (this.fTgtDevice()?.nativeElement as any)?.value as string;
    const tgtPort   = (this.fTgtPort()?.nativeElement as any)?.value as string;
