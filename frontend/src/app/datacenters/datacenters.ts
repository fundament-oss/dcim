...
export default class DatacentersComponent {
  ...
  readonly dcRacks = computed(() =>
    RACKS.filter(r => r.dcId === this.selectedDcId()),
  );
  ...
  readonly dcStats = computed(() => {
    ...
    const totalPowerW  = racks.reduce(
      (sum, r) => sum + r.devices.reduce((s, d) => s + (d.ipmi?.averageW ?? 0), 0), 0,
    );
    const totalUsedU    = racks.reduce((sum, r) => sum + r.devices.reduce((s, d) => s + d.uSize, 0), 0);
  ...
  isoPoints(cell: RackCell, rowIndex: number): {
    ...
    const originY = 80
      + rowIndex       * (this.CELL_D / 2 + this.ROW_GAP)
      + (cell.col - 1) * (this.CELL_D / 2);
  ...
  openCreateDc(): void {
    this.editForm.set({
      id: '', name: '', fullName: '', city: '', country: '', address: '',
      tier: 3, established: new Date().getFullYear(),
      status: 'operational', floorSqm: 0, powerCapacityKw: 0, coolingCapacityKw: 0, pue: 1.5,
    });
  }
