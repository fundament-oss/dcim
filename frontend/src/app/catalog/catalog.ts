...
export class CatalogComponent {
  ...
  readonly categories: AssetCategory[] = [
    'Server', 'Switch', 'Storage', 'Power', 'Firewall', 'Cooling', 'KVM',
    'Memory', 'Disk', 'NIC', 'PSU', 'CPU', 'GPU', 'Transceiver', 'Other',
  ];
  ...
  private readonly allRows = computed<CatalogRow[]>(() =>
    this.mutableCatalog().map(entry => {
      ...
      return {
        ...
        issues:   assets.filter(a => a.status === 'needs-repair' || a.status === 'decommissioned').length,
      };
    }),
  );
  ...
  readonly rows = computed<CatalogRow[]>(() => {
    ...
    return this.allRows().filter(row => {
      ...
      if (q && !row.entry.model.toLowerCase().includes(q) && !row.entry.manufacturer.toLowerCase().includes(q)) return false;
      return true;
    });
  });
